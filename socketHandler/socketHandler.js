...
function emitNotification(userId, data) {
    ...
    if (socketId && io) {
        ...
        console.log(`🔔 Sent real-time to ${userId}`);
    }
}
...
function socketHandler(io) {
    io.on('connection', socket => {
        ...
        socket.on(CHAT, ({ sentBy, receivedBy }) => {
            console.log('New Message :', sentBy, receivedBy);
            let emitToUserId = connectedUsers.get(receivedBy);
            let emitToSender = connectedUsers.get(sentBy);
            emitToUserId && io.to(emitToUserId).emit(CHAT, { sentBy });
            emitToSender && io.to(emitToSender).emit(CHAT, { sentBy });
        });
